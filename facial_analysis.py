...
class FacialImageProcessing:
    ...
    def __init__(self, print_stat=False, mtcnn_detector=True, minsize = 32):
        ...
        model_files={os.path.join('/content/drive/My Drive/Diploma/test_age','mtcnn.pb'):''}
        if age_gender_one_model:
            model_files[os.path.join(models_path,'/content/drive/My Drive/Diploma/test_age/age_gender_tf2_224_deep-03-0.13-0.97.pb')]='' #age_gender_tf2_224_deep_fn-01-0.15-0.98
    ...
    def rename_file(self, filename):
        ...
        _, _,ages,_,_=self.process_image(img)
        age = int(ages[0])
        if age < 11:
            age = 11
        new_name = str(age) + old_name[2:]
...
def rename_images():
    ...
    images_path = "/content/drive/My Drive/Diploma/dataset/age_batches"
    age_batches = os.listdir(images_path)
    for age_file in age_batches:
        path_to_file = os.path.join(images_path, age_file)

        f = open(path_to_file, 'r')
        pictures = [pic for pic in f.read().split('\n') if len(pic) > 0]

        for picture in pictures:
            new_name = imgProcessing.rename_file(picture)
            full_new_name = "'/content/drive/My Drive/Diploma/dataset/new_CACD/'" + new_name
            comand = "cp '" + picture + "' " + full_new_name
            os.system(comand)
...
def get_age():
    ...
    unknown_people_path = "unknown_people/"

    images_file_path = "new_face_rec.txt"
    images_file = open(images_file_path)

    age_file_path = "face_age.txt"
    age_file = open(age_file_path, "w")
    ...
    for line in images_file:
        ...
        expected_path = os.path.join(unknown_people_path, expected)
        real_path = os.path.join(known_people_path, real)

        expected_img = cv2.imread(expected_path)
        real_img = cv2.imread(real_path)
        
        _, _,expected_ages,_,_ = imgProcessing.process_image(expected_img)
        if len(expected_ages) == 1:
            expected_age = int(expected_ages[0])
        else:
            continue

        _, _,real_ages,_,_ = imgProcessing.process_image(real_img)
        if len(real_ages) == 1:
            real_age = int(real_ages[0])
        else:
            continue

        age_info_list = [expected, str(expected_age), real, str(real_age)]
        age_info = ",".join(age_info_list) + "\n"
        age_file.write(age_info)
